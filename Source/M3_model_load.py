...
# 저장된 모델 불러오기
model_load_path = 'D:/ansys/new/Max_model/Max_lstm_model.h5'  # 저장된 모델 경로
...
#%% 예측
ONE_TEMPERATURES = 75
# 75도 데이터만 로드하여 예측을 준비
data_75, temperatures_75, times_75 = load_temperature_data(DATA_DIR_BASE, [ONE_TEMPERATURES], TOTAL_SIZE)
data_75_scaled = scaler_data.transform(data_75)
temps_75_scaled = scaler_temps.transform(temperatures_75)
times_75_scaled = scaler_times.transform(times_75)

combined_75 = np.hstack([temps_75_scaled, times_75_scaled])
X_75, _ = make_samples(combined_75, data_75_scaled, WINDOW_SIZE)

# 180초부터 300초까지의 예측
if len(X_75) >= 180-WINDOW_SIZE:  # 180 - WINDOW_SIZE
    future_predictions = []

    # Start predicting from 180 to 300
    for step in range(FUTURE_STEPS):
        prediction = model.predict(X_75[-1].reshape(1, WINDOW_SIZE, 2))
        future_predictions.append(prediction[0][0])

        # Append the new prediction to the data for subsequent prediction
        next_time = times_75_scaled[-1][0] + (step + 1) / TOTAL_SIZE  # increment time
        new_input = np.array([[temps_75_scaled[-1][0], next_time]]).reshape(1, 2)  # Advance time
        X_75 = np.append(X_75, [np.vstack((X_75[-1][1:], new_input))], axis=0)
    ...
    time_range_75 = np.arange(181, 301)
    # 결과 시각화
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(1, len(data_selected) + 1), data_selected, label=f'Actual Data for {ONE_TEMPERATURES} degrees', linestyle='-')
    plt.plot(time_range_75, future_predictions_original, label=f'Predicted Data for {ONE_TEMPERATURES} degrees',
             linestyle='--')
